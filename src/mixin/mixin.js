...
export default {
	...
	methods: {
		ajax({
			...
			name:name,//名字
			params: params,//参数
			closeloadingOpen:loadingOpen,//真:表示单独关闭打开loading弹窗
			closeloadingClose:loadingClose,//真:表示单独关闭关闭loading弹窗
			closeAllLoading:closeAllLoading,//真:上面两个都不起作用，关闭所有loading
			loadingName:loadingName,//loading名字
			type: type,//post,get提交参数
			errormsg:error,//false,true真的时候返回错误信息
			...
			loadingOpen?loadingOpen=true:loadingOpen=false;
			loadingClose?loadingClose=true:loadingClose=false;
			if(closeAllLoading){
				loadingOpen=true
				loadingClose=true
			}
			loadingName?loadingName=loadingName:loadingName='加载中，请稍后...'
